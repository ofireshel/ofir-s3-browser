export async function onRequestPost({ request, env }) {
  try {
    ...
    const primaryModel = (env.EMOJI_MODEL_ID || 'gpt-5-mini-2025-08-07');
    ...
    async function runModel(modelId, messages, json = true){
      const p = { model: modelId, messages, reasoning_effort: 'minimal' };
...
Output STRICT JSON: { "sentence": "..." }`;
      ...
      for (const mid of gpt5Candidates) { try { resp = await runModel(mid, [ { role:'system', content: sys }, { role:'user', content: user } ], true); if (resp.ok) { usedModelId = mid; break; } } catch {}
      }
      if (!resp || !resp.ok) { resp = await runModel(fallbackModel, [ { role:'system', content: sys }, { role:'user', content: user } ], true); usedModelId = fallbackModel; if (!resp.ok) { const errText = await resp.text(); return new Response(JSON.stringify({ error: 'OpenAI error', details: errText.slice(0, 1000) }), { status: resp.status || 502, headers: { 'Content-Type': 'application/json' } }); } }
